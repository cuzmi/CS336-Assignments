...
class SimpleShardedOptimizer(Optimizer):
    def __init__(self, params: Iterable[torch.nn.Parameter], optimizer: Type[Optimizer], **kwargs: Any):
        self.rank = dist.get_rank()
        self.world_size = dist.get_world_size()


        # NOTE: params生成器只能遍历一次，重复使用请用list - [By: Weijie] - 2026/04/03
        params = list(params)
        ...
        # 存在两个optimizer，一个是self, 一个是inner optimizer
        self.inner_optimizer = optimizer(params, **kwargs)
...
class ShardedOptimizerC(Optimizer):
    def __init__(self, params: Iterable[torch.nn.Parameter], optimizer_cls: Type[Optimizer], **kwargs: Any):
        ...
        # TODO: 这里不是已经传入超参数了，为什么下面的inner还要再传一遍 - [By: Weijie] - 2026/04/03

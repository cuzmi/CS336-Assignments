...
def run_scaled_dot_product_attention(
    ...
    """
    return torch_layers.DotProdAttention(Q, K, V, mask)
...
def run_multihead_self_attention(
    ...
    """
    mla = torch_layers.CausalMultiHeadAttention(d_model, num_heads)
    # 对weight 操作，注意计算图
    with torch.no_grad():
        mla.Q.copy_(q_proj_weight)
        mla.K.copy_(k_proj_weight)
        mla.V.copy_(v_proj_weight)
        mla.W_o.copy_(o_proj_weight)
...
def run_rope(
    ...
    """
    rope = torch_layers.RoPE(theta=theta, d_k=d_k, max_seq_len=max_seq_len, device=in_query_or_key.device)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    nn_utils.grad_l2_norm_clip(parameters, max_l2_norm)
...
def run_save_checkpoint(
    ...
    """
    return extra_component.save_checkpoint(model, optimizer, iteration, out)
...
def run_load_checkpoint(
    ...
    return extra_component.load_checkpoint(src, model, optimizer)
...
def get_tokenizer(
    ...
    """
    return tokenizer.GPT2Tokenizer(vocab, merges, special_tokens)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return train_bpe.run_train_bpe(input_path, vocab_size, special_tokens, **kwargs)
